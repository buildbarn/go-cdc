...
print("Chunk count before deduplication: ", chunks)
print("Total chunk size before deduplication: ", total_chunk_size_before_deduplication)
print(
    "Average chunk size before deduplication: ",
    total_chunk_size_before_deduplication / chunks,
)


total_chunk_size_after_deduplication = 0
chunk_sizes_after_deduplication = Counter()
for _, size_bytes in unique_chunks:
    total_chunk_size_after_deduplication += size_bytes
    chunk_sizes_after_deduplication[size_bytes] += 1
print("Chunk count after deduplication: ", len(unique_chunks))
print("Total chunk size after deduplication: ", total_chunk_size_after_deduplication)
print(
    "Average chunk size after deduplication: ",
    total_chunk_size_after_deduplication / len(unique_chunks),
)
