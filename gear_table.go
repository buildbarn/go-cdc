...
// GearTable is a table of 256 seemingly random 64-bit integers. These
// values are used by various implementations of ContentDefinedChunker
// to compute Gear hashes.
type GearTable struct {
	values [256]uint64
}

// FastContentDefinedChunkerGearTable contains constants that match
// those used by various implementations of the FastCDC algorithm:
//
// - https://github.com/nlfiedler/fastcdc-rs (Rust)
// - https://github.com/buildbuddy-io/fastcdc2020 (Go)
// - https://github.com/HIT-HSSL/destor/blob/master/src/chunking/fascdc_chunking.c (C)
...
// NewSeededGearTable creates a GearTable that is initialized with
// values that are based on a seed.
//
// The seed is hashed using cSHAKE128, and the constants are set to the
// first 2 KiB of data generated by the XOF, interpreted as a sequence
// of little-endian integers.
func NewSeededGearTable(seed []byte) *GearTable {
	seedHash := sha3.NewCSHAKE128(nil, []byte(`c647d5de-0fff-40e6-963c-814bc404a64d`))
	if _, err := seedHash.Write(seed); err != nil {
		panic(err)
	}
	gt := &GearTable{}
	for i := 0; i < len(gt.values); i++ {
		var v [8]byte
		if _, err := seedHash.Read(v[:]); err != nil {
			panic(err)
		}
